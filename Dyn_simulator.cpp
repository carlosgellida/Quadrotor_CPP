...
xtensor<float, 1> q_part({int(Nj)}) ; // Initial joint coordinates
...
/* This function provides InfoI, which contains one 3x3 matriz per each
  ...
  view(Info1, 3, 2, all()) = xarray<float> ({-1.5708, 0, 0}) ;  //-90 degrees rotation over x axis

  view(Info1, 4, 0, all()) = xarray<float> ({1.36561e-05, 1.36771e-05, 2.13783e-08}) ; 
  view(Info1, 4, 2, all()) = xarray<float> ({-1.5708, 0, 0}) ;  //-90 degrees rotation over x axis

  view(Info1, 5, 0, all()) = xarray<float> ({1.36561e-05, 1.36771e-05, 2.13783e-08}) ; 
  view(Info1, 5, 2, all()) = xarray<float> ({-1.5708, 0, 0}) ;  //-90 degrees rotation over x axis

  view(Info1, 6, 0, all()) = xarray<float> ({1.36561e-05, 1.36771e-05, 2.13783e-08}) ;
  view(Info1, 6, 2, all()) = xarray<float> ({-1.5708, 0, 0}) ;  //-90 degrees rotation over x axis 
...
int main(){
  ...
  xtensor_fixed<float, xshape<6, 6>> Ext_R = zeros<float> ({6, 6}) ; 
  xtensor_fixed<float, xshape<6>> V_inertial = zeros<float> ({6}) ; 
  xtensor_fixed<float, xshape<6>> V_body = zeros<float> ({6}) ; 
  xtensor<float, 2> V_inertial_record({size_t(n_step), 6}) ; 
  xtensor<float, 2> V_body_record({size_t(n_step), 6}) ; 
  ...
  for (int h = 0; h < n_step; h++){
    ...
    view(Ext_R, range(0, 3), range(0, 3)) = view(Chi_base, range(0, 3), range(0, 3)) ; 
    view(Ext_R, range(3, 6), range(3, 6)) = view(Chi_base, range(3, 6), range(3, 6)) ; 
    
    V_body = view(dot_q, range(0, 6)) ; 
    V_inertial = dot(Ext_R, V_body ) ; 
    
    view(V_inertial_record, h, all()) = V_inertial ; 
    view(V_body_record, h, all()) = V_body ; 
  ...
  fstream my_file3;
  my_file3.open("V_inertial_record.txt", ios::out);
  xt::dump_csv(my_file3, V_inertial_record);
  my_file3.close();
  
  fstream my_file4;
  my_file4.open("V_body_record.txt", ios::out);
  xt::dump_csv(my_file4, V_body_record);
  my_file4.close();
  ...
  dot_q_0 = zeros<float> ({int(DoF)}) ; // Generalized velocities
  //dot_q_0(10) = 0.1 ; 

  q_part_0 = zeros<float> ({(int(Nj) + 3)}) ; // Initial joint coordinates t = 0

  R_base_0 = eye<float> (3) ; // Orientation of base body w.r.t. inertial t = 0

  P_base_0 = ones<float> ({1, 3}) ; //Position of base body w.r.t. inertial t = 0

  Chi_base_0 = zeros<float> ({6, 6}) ; 
  view(Chi_base_0, range(0, 3), range(0, 3)) = R_base_0 ; 
  view(Chi_base_0, range(0, 3), range(3, 6)) = dot(R_base_0, skew3(P_base_0) );
  view(Chi_base_0, range(3, 6), range(3, 6)) = R_base_0 ;
  view(Chi_base_0, range(3, 6), range(0, 3)) = zeros<float> ({3, 3}) ; 
  ...
  omega = zeros<float> ({3}) ;
  delta_q_base = zeros<float> ({6}) ;
  Chi_int = zeros<float> ({6, 6}) ; 
  delta_q_part = zeros<float> ({int(Nj) + 3}) ; 
  delta_p = zeros<float> ({3}) ;
  ...
  time_record = zeros<int> ({int(n_step)}) ; // Time vector for plotting
  ...
  time = double(clock()) - time ; 
  time_taken =  time/double(CLOCKS_PER_SEC);
  cout << "The process 2 takes: " << time_taken << "s" << endl ; 
  cout << q_part_record << endl ; 
  ...
  fstream my_file2;
  my_file2.open("q_part_record2.txt", ios::out);

  xt::dump_csv(my_file2, q_part_record);
