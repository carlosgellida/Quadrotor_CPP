...
q_part_0[5] = 3.14159/4 ;
q_part_0[6] = 3.14159/4 ;
...
for i in range(0,n):
    ...
    #Chi_base = Chi_base@expm(skew61(dot_q[:6]*h))
    print("Made = ", i*100/n, "%")


'''
dot_q_euler = np.array([0, 0, 0])
q_euler = np.array([0, 0, 0])
phi, theta, psi = (0, 0, 0)

for i in range(0,n):
    t = i*h
    
    k1, V_frame0 = direct_dynamic(dot_q, Chi_base, q)
    k2, Vfake = direct_dynamic(dot_q + 0.5*k1*h, Chi_base, q)
    k3, Vfake = direct_dynamic(dot_q + 0.75*k2*h, Chi_base, q)
    dot_q = dot_q + (2/9)*k1*h + (1/3)*k2*h + (4/9)*k3*h
    
    J_theta = np.array([[cos(theta), -sin(theta)*sin(phi), cos(phi)*sin(theta)], 
                        [0, cos(phi)*cos(theta), -sin(phi)*cos(theta)], 
                        [0, sin(phi), cos(phi) ]   ])
    

    
    #dot_q_euler = np.linalg.inv(J_theta) @ Chi_base[:3, :3].T @ dot_q[3:6]
    dot_q_euler = np.linalg.inv(J_theta) @ dot_q[3:6]
    dot_q2 = np.copy(dot_q)
    dot_q2[3:6] = dot_q_euler
    
    k1b = dot_q2
    k2b = dot_q2 + 0.5*k1b*h
    k3b = dot_q2 + 0.75*k2b*h
    
    
    q = q + (2/9)*k1b*h + (1/3)*k2b*h + (4/9)*k3b*h
    
    phi, theta, psi = list(q[3:6])
    
    Chi_x = np.identity(3) + sin(phi)*skew3(np.array([1, 0, 0])) + (1 - cos(theta))*skew3(np.array([1, 0, 0]))@skew3(np.array([1, 0, 0]))
    Chi_y = np.identity(3) + sin(theta)*skew3(np.array([0, 1, 0])) + (1 - cos(theta))*skew3(np.array([0, 1, 0]))@skew3(np.array([0, 1, 0]))
    Chi_z = np.identity(3) + sin(psi)*skew3(np.array([0, 0, 1])) + (1 - cos(theta))*skew3(np.array([0, 0, 1]))@skew3(np.array([0, 0, 1]))
    
    Chi_base[3:, 3:] = Chi_z @ Chi_y @ Chi_x
    Chi_base[:3, :3] = np.copy(Chi_base[3:, 3:])
    Chi_base[:3, 3:] = Chi_base[3:, 3:] @ skew3(q[:3])
    
    R_base[3:, 3:] = np.copy(Chi_base[3:, 3:])
    R_base[:3, :3] = np.copy(Chi_base[3:, 3:])
    
    dot_q_save = np.vstack((dot_q_save, dot_q))
    V_save = np.vstack((V_save, V_frame0))
    q_save = np.vstack((q_save, q))
    #if i == 220:
    #    break
'''
